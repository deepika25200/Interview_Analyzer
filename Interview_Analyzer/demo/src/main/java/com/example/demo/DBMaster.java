...
public class DBMaster {
    ...
    public static void datainsertion(List<JavaDataModel> data) {
        data.parallelStream().forEach(tuple -> {
            try (Connection connection = dataSource.getConnection();
                ...
                preparedStatement.setString(1, tuple.getDate());
    ...
    public static void MaxInterviewsTeam() {
        try (Connection connection = dataSource.getConnection();
             Statement statement = connection.createStatement()) {
            String query = "SELECT team, COUNT(*) as maxCount\n" +
                    "FROM interview_data\n" +
    ...
    public static void MinInterviewsTeam() {
        try (Connection connection = dataSource.getConnection();
             Statement statement = connection.createStatement()) {
            String query = "SELECT team, COUNT(*) as minCount\n" +
                    "FROM interview_data\n" +
                    "WHERE month IN ('Oct-23', 'Nov-23')\n" +
                    "GROUP BY team\n" +
                    "ORDER BY minCount \n" +
    ...
    public static void getTop3Skills() {
        try (Connection connection = dataSource.getConnection();
            ...
            String query = "CREATE OR REPLACE VIEW skills_view AS\n" +
                    "SELECT skill, COUNT(*) as skillsCount\n" +
                    "FROM interview_data\n" +
                    "WHERE month IN ('Oct-23', 'Nov-23')\n" +
                    "GROUP BY skill\n" +
            ...
            while (resultSet.next()) {
                System.out.println("Skill name: " + resultSet.getString("skill") +
                        ", Candidates Count: " + resultSet.getInt("skillsCount"));
            }
